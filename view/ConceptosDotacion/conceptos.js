const CONCEPTOS_URL = '../../controller/conceptosdotacion.php';
...
$(document).ready(function () {
    tablaConceptos = $('#tbConceptos').DataTable({
        ...
        ajax: {
            ...
            type: 'POST',
            data: function (d) { d.busqueda = $('#inputBusqueda').val().trim(); }
        },
        columns: [
...
function buscar() { tablaConceptos.ajax.reload(); }
...
function abrirModalEditar(id) {
    $.ajax({
        ...
        success: function (resp) {
            if (resp.status !== 'success') { swal('Error', resp.message, 'error'); return; }
            var c = resp.data;
            $('#conceptoId').val(c.id);
        ...
        error: function () { swal('Error', 'No se pudo cargar el concepto.', 'error'); }
    });
}
...
function guardarConcepto() {
    ...
    if (esEdicion) { datos.id = id; datos.estado = estado; }

    $('#btnGuardarConcepto').prop('disabled', true)
                            .html('<i class="fa fa-spinner fa-spin"></i> Guardando...');

    $.ajax({
        url:      CONCEPTOS_URL + '?op=' + (esEdicion ? 'editar' : 'crear'),
        type:     'POST',
        data:     datos,
        dataType: 'json',
        success: function (resp) {
...
function cambiarEstado(id, nuevoEstado, nombre) {
    ...
    swal({
        ...
        $.ajax({
            ...
            error: function () { swal('Error', 'Error de comunicación con el servidor.', 'error'); }
        });
    });
}
